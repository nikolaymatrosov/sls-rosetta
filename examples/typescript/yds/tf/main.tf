# Build TypeScript code
resource "null_resource" "build_typescript" {
  provisioner "local-exec" {
    command     = "cd ../function && npm install && npm run build"
    working_dir = path.module
  }

  triggers = {
    # Rebuild when source files change
    producer_hash = filesha256("../function/producer.ts")
    consumer_hash = filesha256("../function/consumer.ts")
    package_hash  = filesha256("../function/package.json")
    tsconfig_hash = filesha256("../function/tsconfig.json")
  }
}

# Archive built function code
data "archive_file" "function_files" {
  type        = "zip"
  source_dir  = "../dist"
  output_path = "./function.zip"
  excludes = [
    "node_modules",
    "*.ts",
    "tsconfig.json"
  ]

  depends_on = [null_resource.build_typescript]
}

# Producer Function
resource "yandex_function" "producer" {
  name               = "yds-producer-ts"
  description        = "Producer function that writes messages to YDS topic (TypeScript)"
  user_hash          = data.archive_file.function_files.output_sha256
  runtime            = "nodejs22"
  entrypoint         = "producer.handler"
  memory             = 128
  execution_timeout  = "10"
  service_account_id = yandex_iam_service_account.producer_sa.id

  environment = {
    YDB_ENDPOINT   = yandex_ydb_database_serverless.yds_demo.ydb_full_endpoint
    YDS_TOPIC_PATH = "${yandex_ydb_database_serverless.yds_demo.database_path}/${yandex_ydb_topic.yds_demo_topic.name}"
  }

  content {
    zip_filename = data.archive_file.function_files.output_path
  }

  depends_on = [
    yandex_resourcemanager_folder_iam_member.producer_roles
  ]
}

# Consumer Function
resource "yandex_function" "consumer" {
  name               = "yds-consumer-ts"
  description        = "Consumer function that processes messages from YDS topic (TypeScript)"
  user_hash          = data.archive_file.function_files.output_sha256
  runtime            = "nodejs22"
  entrypoint         = "consumer.handler"
  memory             = 128
  execution_timeout  = "10"
  service_account_id = yandex_iam_service_account.consumer_sa.id

  content {
    zip_filename = data.archive_file.function_files.output_path
  }

  depends_on = [
    yandex_resourcemanager_folder_iam_member.consumer_roles
  ]
}

# YDS Trigger
resource "yandex_function_trigger" "yds_trigger" {
  name        = "yds-trigger-ts"
  description = "Trigger that invokes consumer function on YDS messages (TypeScript)"
  folder_id   = var.folder_id

  function {
    id                 = yandex_function.consumer.id
    service_account_id = yandex_iam_service_account.trigger_sa.id
    retry_attempts     = 3
    retry_interval     = 10
  }

  data_streams {
    stream_name        = yandex_ydb_topic.yds_demo_topic.name
    database           = yandex_ydb_database_serverless.yds_demo.database_path
    service_account_id = yandex_iam_service_account.trigger_sa.id
    batch_cutoff       = 5
    batch_size         = 10
  }

  depends_on = [
    yandex_resourcemanager_folder_iam_member.trigger_roles,
    yandex_ydb_topic.yds_demo_topic
  ]
}
