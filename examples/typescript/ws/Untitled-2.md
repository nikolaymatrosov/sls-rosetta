...
Сообщения, отправленные клиентам (например, с помощью вызова gRPC API  [Send](https://yandex.cloud/ru/docs/api-gateway/apigateway/websocket/api-ref/grpc/Connection/send)), и пинги внутри WebSocket-соединений не тарифицируются.

## [](https://yandex.cloud/ru/docs/api-gateway/concepts/extensions/websocket#connect)Операция x-yc-apigateway-websocket-connect
...
Для каждого нового веб-сокета генерируется уникальный идентификатор соединения, который возвращается клиенту в заголовке  `X-Yc-Apigateway-Websocket-Connection-Id`. Идентификатор соединения передается при вызове интеграции. Чтобы управлять установленным соединением — отправлять данные на клиентскую сторону или закрывать соединение — с помощью  [API](https://yandex.cloud/ru/docs/api-gateway/api-ref/websocket/authentication), сохраните полученный идентификатор, например в базе данных  [Yandex Managed Service for YDB](https://yandex.cloud/ru/docs/ydb).
...
Если в качестве интеграции используется функция Yandex Cloud Functions, перечисленная выше информация про веб-сокет передается в виде отдельных полей внутри  `requestContext`  [JSON-структуры](https://yandex.cloud/ru/docs/functions/concepts/function-invoke#request)  запроса к функции.

Для данной операции можно настроить  [авторизацию с помощью функции](https://yandex.cloud/ru/docs/api-gateway/concepts/extensions/function-authorizer). Если авторизация будет неуспешной, соединение не установится и клиент получит ответ с HTTP-кодом  `401`  или  `403`.
...
- Отправлять сообщения на клиентскую сторону через  [API управления соединениями](https://yandex.cloud/ru/docs/api-gateway/api-ref/websocket/authentication).
- Сохранять идентификатор соединения и другую информацию в базы данных.

## [](https://yandex.cloud/ru/docs/api-gateway/concepts/extensions/websocket#message)Операция x-yc-apigateway-websocket-message

Операция выполняется, когда отправляется сообщение с клиентской стороны. API Gateway, выполняя ее, вызывает интеграцию. Данные, полученные из веб-сокета, передаются в теле HTTP-запроса к интеграции. Поддерживаются текстовые (кодировка UTF-8) и бинарные сообщения согласно  [RFC](https://www.rfc-editor.org/rfc/rfc6455#section-5.6). При текстовых данных в заголовок  `Content-Type`  проставляется значение  `application/json`, при бинарных —  `application/octet-stream`. Если в качестве интеграции используется функция Cloud Functions, для бинарного сообщения выполняется base64-кодирование данных и получившееся строковое значение записывается в поле  `body`  [JSON-структуры](https://yandex.cloud/ru/docs/functions/concepts/function-invoke#request)  запроса, а флаг  `isBase64Encoded`  проставляется в  `true`.
...
Если в качестве интеграции используется функция Cloud Functions, перечисленная выше информация про сообщение передается в виде отдельных полей внутри  `requestContext`  [JSON-структуры](https://yandex.cloud/ru/docs/functions/concepts/function-invoke#request)  запроса к функции.
...
## [](https://yandex.cloud/ru/docs/api-gateway/concepts/extensions/websocket#disconnect)Операция x-yc-apigateway-websocket-disconnect
...
Если в качестве интеграции используется функция Cloud Functions, перечисленная выше информация про закрытое соединение передается в виде отдельных полей внутри  `requestContext`  [JSON-структуры](https://yandex.cloud/ru/docs/functions/concepts/function-invoke#request)  запроса к функции.
...
## [](https://yandex.cloud/ru/docs/api-gateway/concepts/extensions/websocket#spec)Спецификация расширения
